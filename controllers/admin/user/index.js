...
module.exports = function(app) {
    ...
    app.get('/baseInfo', auth.isAuthenticated('ROLE_USER'), function(req, res, next) {
        var user = req.user;
        var model = {
        ...
        UserInfo.findOne({
            ...
            model.title = '维护用户信息';
            model.user = req.user;
            res.render('admin/user/add', model);
        });
    });

    app.post('/baseInfo', auth.isAuthenticated('ROLE_USER'), function(req, res, next) {
            ...
            UserInfo.findOne({
                ...
                userInfoModel.save(function(err, userInfo) {
                    if (err) {
                        ...
                        res.locals.err = err;
                        res.locals.view = 'admin/user/add';
                    ...
                    user.save(function(err) {
                        ...
                        res.redirect('/admin/user/baseInfo');
                    });
                });
            });
        });
    ...
    app.get('/resetPassword', auth.isAuthenticated(), function(req, res, next) {
        var model = {
            title: '重置密码',
            showMessage: req.flash('showMessage'),
            showErrorMessage: req.flash('showErrorMessage')
        };
        res.render('admin/user/resetPassword', model);
    });
    app.post('/user/resetPassword', auth.isAuthenticated(), function(req, res, next) {
        ...
        if (oldPassword.trim() === newPassword.trim()) {
            ...
            res.redirect('/admin/user/resetPassword');
        }
        User.findOne({
            name: req.user.name
        }, function(err, user) {
